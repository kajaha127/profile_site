...
$(function () {
	...
	$(window).on('scroll', function () {
		...
		for (let i = 0; i < $mnu.length; i++) {
			if (scrollTop >= arrTopVal[i] - headerH - 100) {
				$mnu.eq(i).parent().addClass('on').siblings().removeClass('on')
			} else if (scrollTop < arrTopVal[0] - headerH - 100) {
...
// 제작과정 모달창
$(function () {
	const $portfolioContainer = $('#portfolio > .portfolio-containers > .portfolio-container')
	...
	$btnProcess.on('click', function (evt) {
		evt.preventDefault()
		if (
			$(this).parents('.show').siblings('.shadow').find('img').attr('alt') === 'CJ 뉴스룸 제작과정'
		) {
			$shadowCJ.show()
		} else if (
			$(this).parents('.show').siblings('.shadow').find('img').attr('alt') === '써브웨이 제작과정'
		) {
			$shadowSub.show()
		} else if (
			...
			$(this).parents('.show').siblings('.shadow').find('img').attr('alt') === '룩핀 제작과정'
		) {
			$shadowLP.show()
		}
	})
